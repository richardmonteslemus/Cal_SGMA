---
title: "California SGMA Analysis"
...
Temporal: Current groundwater levels depend on previous levels, meaning observations are not independent over time. This creates autocorrelation in the data.

Autocorrelation not addressed: This analysis does not account for autocorrelation in the regression model. Therefore, groundwater level observations cannot be considered independent. This can affect the reliability of standard errors and significance tests.

Policy Intervention Variation: Not all wells were subject to the January 2020 SGMA Sustainable Groundwater Plan submission and implementation. Some wells had until January 2022. Despite this, a single policy intervention date (January 2020) was used in order to make all groundwater levels observations comparable to each other.
...
gw_level_month <- read_csv(here("data", "16f256f8-35a4-4cab-ae02-399a2914c282.csv"))

# Read in groundwater station metadata
station <- read_csv((here("data", "03967113-1556-4100-af2c-b16a4d41b9d0.csv")))
```
# Cleaning data 
...
station_gw_filtered <- station_gw_filtered %>%
...
**Breakpoint:**
The model calculated the breakpoint to be month 126.86 which is approximately September 2022. 

**Confidence Interval:**
Time: 0.12 feet/month
The standard error corresponds to the pre-breakpoint slope. We are 95% confident the slope is between 0.07 and 0.55 feet per month. Since zero is not included, we are confident groundwater levels were rising before the breakpoint. 

U1.Time: 0.72 feet/month
The standard error corresponds to the post-breakpoint slope. Since we did not receive a P value for this slope we are unable to make claims about the confidence in the slope range. 

Breakpoint: 1.534 months 
The standard error corresponds to the breakpoint estimate. We are 95% confident the breakpoint falls between month 124 - 130 (July 2022 - November 2022). 
...
ggplot() +
  ...
  scale_color_manual(values = c("Pre-breakpoint" = "cornflowerblue", 
                     ...
                     name = "Time Period") +
  labs(
    title = "Segmented Regression Model: Observed vs. Predicted Groundwater Levels",
    x = "Date",
